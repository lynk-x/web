...
interface WalletTableProps {
    ...
    onSelectAll?: () => void;
    onFreeze?: (accountId: string, currency: string, currentStatus: string) => void;
    onAdjustBalance?: (accountId: string, currency: string) => void;
}
    ...
    const getActions = (w: AdminWallet): ActionItem[] => [
        {
            ...
            onClick: () => {
                // Future: Navigate to wallet-ledger detail filtered by account
            }
        },
        {
            ...
            onClick: () => {
                if (onFreeze) onFreeze(w.account_id, w.currency, w.status);
            },
            variant: w.status === 'active' ? 'danger' : 'success'
        },
        {
            label: 'Adjust Balance',
            icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>,
            onClick: () => {
                if (onAdjustBalance) onAdjustBalance(w.account_id, w.currency);
            }
        }
    ];
