...
export default function FeatureFlagTab({
    ...
    const fetchFlags = useCallback(async () => {
        setIsLoading(true);
        try {
            const { data, error } = await supabase.schema('api').rpc('get_feature_flags');
    ...
    useEffect(() => {
        if (triggerCreate > 0) {
            showToast('Flag creation moved to inline form. Use the Settings page.', 'info');
        }
    }, [triggerCreate, showToast]);

    const handleToggleFlag = async (key: string, currentValue: boolean) => {
        try {
            const { error } = await supabase.schema('api').rpc('toggle_feature_flag', {
                p_key: key,
