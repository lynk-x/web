 ...
 * registration, and Google OAuth SSO. Rendered by both /login and /signup —
 * `initialMode` picks which copy/fields show by default, but the two are
 * still distinct routes (not a same-page toggle) so each has its own URL,
 * can be linked to directly, and shows up separately in analytics.
 *
...
export default function AuthPage({ initialMode }: { initialMode: 'login' | 'signup' }) {
    ...
    const isLoginDetail = initialMode === 'login';
    ...
    // Clear out errors when switching modes
    useEffect(() => {
