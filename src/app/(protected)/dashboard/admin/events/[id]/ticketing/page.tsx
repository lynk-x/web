...
/**
 * Admin sub-page for an event's ticketing & resale oversight — split out of
 * the event detail page's former "Ticketing & Resale" tab into a real
 * sub-route, matching the organizer dashboard's event detail layout.
 */
export default function AdminEventTicketingPage(props: { params: Promise<{ id: string }> }) {
    return (
        <Suspense fallback={<div className={adminStyles.container}><Spinner label="Loading..." centered /></div>}>
            <AdminEventTicketingContent {...props} />
        </Suspense>
    );
}

function AdminEventTicketingContent({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
